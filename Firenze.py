...
hub_firenze = st.sidebar.selectbox(
    'Hub Firenze', ['WhatsApp', 'Imóvel reservado', 'Imóvel disponível', 'Imóvel indisponível'], 0)
...
if hub_firenze == 'Imóvel reservado':
    def generate_message(nome, email, endereco, link_endereco, link_whatsapp):
        message = f"""Oi, {nome},
        tudo bem? Aqui é Wemerson da Firenze Imóveis, recebi seu e-mail referente ao imóvel em [{endereco}]({link_endereco}), só passando para informar que esse imóvel encontra-se reservado.
        A reserva do imóvel não garante a locação, para ter mais informações e um atendimento mais rápido, envie uma mensagem clicando no nosso [WhatsApp]({link_whatsapp}) Aguardo o seu contato :)"""
        return message
